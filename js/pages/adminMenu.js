...
export function inicializarAdministrador(datosPredeterminados = []) {
    
    const datosLocales = localStorage.getItem("menu");

    if (datosLocales) {
        menuData = JSON.parse(datosLocales);
    } else {
      
        menuData = datosPredeterminados;
